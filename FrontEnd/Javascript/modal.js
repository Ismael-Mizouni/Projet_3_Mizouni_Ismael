...
function createModalWorkFigure(work) {
  ...
  figure.appendChild(deleteIcon)
   ...
   // evénement lors du clic sur delete
...
//Vérifier que le formulaire est remplie//
...
function addNewWork(event) {
  ...
  /* Vérifier si l'image ne dépasse pas 4Mo */
  ...
  .then(response => response.json()) 
  .then(work => {
    /* Créer et ajouter nouveau travail à la gallerie */
    const figure = createWorkFigure(work);
    const gallery = document.querySelector('.gallery');
    gallery.appendChild(figure);
  
    /* Créer et ajouter travail à la modale */
